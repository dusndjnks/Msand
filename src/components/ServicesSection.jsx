...
const ServicesSection = () => {
  ...
  return (
    <motion.section className="relative py-16 bg-gradient-to-b from-gray-50 to-white" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
      {/* Animated HR */}
      <motion.div className="w-full h-0.5 bg-gradient-to-r from-primary to-secondary mb-14" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }} />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div className="text-center mb-14" initial={{ y: -20 }} whileInView={{ y: 0 }} transition={{ duration: 0.5 }}>
          <h2 className="text-4xl font-extrabold text-gray-900 mb-4">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">What We Do</span>
          </h2>
          <motion.p className="text-lg text-gray-600 max-w-2xl mx-auto" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.2 }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div key={index} className="bg-white rounded-xl shadow-md border border-gray-100 p-7" variants={cardVariants} whileHover="hover" transition={{ type: "spring" }}>
              <motion.div className={`w-14 h-14 rounded-lg flex items-center justify-center mb-5 ${service.color === 'primary' ? 'bg-primary/10' : 'bg-secondary/10'}`} variants={iconContainerVariants}>
                <motion.div variants={iconVariants}>
                  <motion.svg className={`w-7 h-7 ${service.color === 'primary' ? 'text-primary' : 'text-secondary'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" variants={svgVariants}>
                    {service.icon}
                  </motion.svg>
                </motion.div>
              </motion.div>
